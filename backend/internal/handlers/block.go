...
func (h *BlockHandler) handleGet(s network.Stream) {
	...
	logrus.Debug("Received block get request: ", string(jsonData))
	...
	logrus.Debug("Got block metadata: ", string(metadata))
...
func (h *BlockHandler) handleStat(s network.Stream) {
	...
	logrus.Debug("Received block stat request: ", string(jsonData))
	...
	if arg, ok := query["arg"]; ok {
		for _, c := range arg {
			...
			if peerInfo == nil {
				...
				logrus.Debug("Received block metadata: ", metadata)
			}
